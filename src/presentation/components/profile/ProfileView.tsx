...
import { MainProfileContent } from './main/MainProfileContent';
import { RetroProfileContent } from './retro/RetroProfileContent';
...
/**
 * ProfileView - Main view component for profile page
...
export function ProfileView({ initialViewModel }: ProfileViewProps) {
  ...
  return layout === 'retro' ? (
    <RetroProfileContent viewModel={viewModel} onSwitchProfile={actions.switchProfile} />
  ) : (
    <MainProfileContent viewModel={viewModel} onSwitchProfile={actions.switchProfile} />
  );
}
  ...
  return (
    <div className="h-full w-full flex items-center justify-center">
      <div className="text-center">
        ...
        <p style={{ fontSize: '11px' }}>Loading profile...</p>
      </div>
    </div>
  );
}
